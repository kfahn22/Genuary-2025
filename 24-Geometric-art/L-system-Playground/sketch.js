...
let ruleChoices = ["levy_curve", "levy_curve"];
let shapeChoices = ["Polygon", "Polygon"];
let strokeChoices = ["blue", "blue"];
let fillChoices = ["purplePalette", "orangePalette"];
...
function evenMoreShapes(type) {
   let s = ceil(21 * 0.7);
   shapes.push(new MoreShapes(width * 0.85, height * 0.15, s, 10));
   shapes.push(new MoreShapes(width * 0.75, height * 0, s, 6));
   shapes.push(new MoreShapes(width * 0.925, height * 0.045, s, 8));
   shapes.push(new MoreShapes(width * 0.03, height * 0.945, s, 8));
   shapes.push(new MoreShapes(width * 0.0, height * 0.745, s, 8));
   for (let s of shapes) {
    if (type == 0)
    { s.alternatingSquares();}
    else {
     s.alternatingCircles();
    }
   }
   shapes[0].grid(width*0.35, height* 0.275, 5)
   shapes[0].grid(width * 0.715, height * 0.725, 5);
}

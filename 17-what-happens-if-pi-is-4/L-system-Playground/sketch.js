// January 17: What happens if pi is 4?
// Note: if have edited most of the rules.json and shpae.js files to reflect the altered value of pi.
...
let ruleChoices = ["island_curve", "island_curve"];
let shapeChoices = ["Astroid", "Astroid"];
let strokeChoices = ["blue", "blue"];
let fillChoices = ["blue_yellow", "bluePalette"];
...
function setSystemVariables(lsystems) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (syncVariables.checked() && n == 2) {
      lsystemValues[1].LsystemValues.sliderValues.systemValues.wadj =
